...
router.get('/shows/:showId/seats', async (req, res) => {
    try {
        ...
        // Get already booked seats for THIS SPECIFIC SHOW
...
router.post('/shows/:showId/book', async (req, res) => {
    ...
    try {
        ...
        // Check if any of the selected seats are already booked for THIS SPECIFIC SHOW
        const checkSeatsQuery = `
            SELECT b."Seat_no"
            FROM "Booked" b
            WHERE b."Show_id" = $1 AND b."Seat_no" = ANY($2::int[])
        `;
        ...
        for (const seatId of selectedSeats) {
            ...
            await client.query(
                ...
                [userId, showData.movie_id, seatId, showId]
            );
        }
...
router.delete('/bookings/:bookingId', async (req, res) => {
    try {
        ...
        // Delete the booking from Booked table
        const deleteQuery = 'DELETE FROM "Booked" WHERE "ID" = $1 RETURNING *';
        const result = await db.query(deleteQuery, [bookingId]);
